...
{
  char *t;
  return (dt[0] == '*' && (t = stristr(nm, dt + 1)) && ! (t[l1 - 2] ) ) || !stricmp(nm, dt);
}
    ...
    else
    {
      ...
      while(first < last)
      {
        if(first->type)
        {
          ...
          else if( ( hash == first->hash && !stricmp(nm, first->dt) ) )
          {
            return first;
          }
        }
...
void FlashRepeatLog()
{
  ...
  for(i = 0; i < REPEAT_Q_SIZE; i++)
  {
    if( logrepeat[i].cnt )
    {
      ...
      else if(logrepeat[i].cnt > 1)log_flash_time = cur_time + 20;
    }
  }
}
...
void LoadDomainM()
{
  if(count_dns)
  {
    ...
    {
      debug("Error loading host definetion file.");
    }
  }
}
...
err:
    ...
    {
#ifdef USE_IPV6
      if(bind_a[SDNS_IND] ? sa_server6.sin6_family == AF_INET6 : is_ip6)
      {
