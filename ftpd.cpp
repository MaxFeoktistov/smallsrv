...
//     unicode2utf(dbg,ut,256);
    // debug("FindFirstFileW(%s)  %s %s %s %X %X %X",a,ut,ut+1,ut+2,ut[0],ut[1],ut[2]);
     ...
     err=GetLastError();
 //    debug("FindFirstFileW(%s) return %d %d %s %s %s |%s| %s",a,r,err,ut,ut+1,ut+2,dbg,strerror(err) );
...
// DBG_MSG("TTXD %d %X",GetLastError(),bb);
 timout=FTPTimeout;
...
//  if((cmd=GetCMD(s,bfr,FTPTimeout))<0)goto ex1;
  ...
  else  switch(cmd)
  {
   ...
   case 0x72657375 x4CHAR("user")://
    ...
    if((puser=FindUser(cpath,UserFTP,0,this)))
    {
      if( (t=puser->pasw()) && !*t)
      {
...
lbCONECT:
    ...
    {if(prti.sin_port && (prti.sin_addr. S_ADDR == /* saddr[ntsk]*/ sa_c.sin_addr. S_ADDR || !(s_flgs[1]&FL1_FTPSAME)
     ...
     {if((s2=socket(prti.sin_family           //AF_INET
    ...
    switch(cmd)
    {
     case 0x72746572 x4CHAR("retr")://13:
      ...
      if( (i=_lopen(bb,0))>0)
      {
       ...
       HResumePos=0;
       do{
          l=_hread(i,t,BBSIZE); if((!l) ||
            //send(s2,t,l,0)
//            (ww=JustSend(t,l)) <l
            (ww=Send(t,l)) <l
          )break;
          x+=l;  //Tout+=l;
          //if(speed[2] && (r=(GetTickCount()-tmout)-Tout/speed[2])>0 )Sleep(r);
          //SleepSpeed() ;//sa_c.sin_addr.s_addr);
         }while(l==BBSIZE);
         /*
       xchg(s,s2);
       if(sec_state&2)
...
//         DBG_MSG("TTXD1 %X %d",bb,HeapSize(GetProcessHeap(),0,bb));
         break;
        }
        _hwrite(i,t,l);
        x+=l;
...
DBG_MSG("LIST4: %.64s %u",bb,l);
    ...
    if((i=t-bb)>0)
    {
//      if(send(s2,bb,i,0)<0)goto erSnd;
